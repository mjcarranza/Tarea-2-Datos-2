...
string Graph::insert_vertice(string nombre) {
...
/* inserting "aristas" */
string Graph::insert_arista(Vertice *origen, Vertice *destino, int peso) {
    ...
    return (origen->nombre, nueva->Adyacente->nombre, std::to_string(origen->ady->peso));
...
int Graph::floyd(int matrix[][9],int fil, int col) {
    int counting =0;
    ...
    for (int k=0; k<fil; k++){ /* k is the pivot */
        for (int i=0; i<fil; i++){
            for (int j=0; j<fil; j++){
